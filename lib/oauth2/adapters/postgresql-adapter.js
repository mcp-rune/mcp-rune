...
export class PostgresqlAdapter extends BaseTokenStoreAdapter {
   ...
   * @param {import('pg').Pool} options.pool - Shared pg.Pool (from src/engineer/db.js)
   */
  constructor({ pool }) {
    super()
    this.pool = pool
  ...
  async storeTokens({ userId, accessToken, refreshToken, expiresIn, scope, mcpSessionId }) {
    this._ensurePool()
  ...
  async getTokens(userId) {
    ...
    return this._rowToTokens(rows[0])
  }

  async getTokensBySession(mcpSessionId) {
    this._ensurePool()
