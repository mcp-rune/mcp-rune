...
export class OAuthService {
   ...
   * @returns {Array|undefined} Execute options array or undefined
   */
  _getExecuteOptions() {
   ...
   *
   * @param {client.Configuration} config - OpenID client configuration
   * @param {string} codeChallenge - PKCE code challenge (RFC7636)
   ...
   *
   * @param {string} mcpSessionId - MCP session identifier
   * @returns {Promise<Object>} User info after successful authentication
   */
  async startLocalAuthFlow(mcpSessionId) {
      ...
      const server = http.createServer(async (req, res) => {
        ...
        if (url.pathname === callbackUrl.pathname) {
          ...
          try {
            ...
            logger.error('Token exchange failed', { service: 'oauth2', error: err.message })
   ...
   * @param {string} codeVerifier - PKCE code verifier from session (RFC7636)
   * @param {string} expectedState - Expected state from session
   * @param {string} mcpSessionId - MCP session identifier
   * @returns {Promise<Object>} User info after successful authentication
   */
  async handleRemoteCallback(callbackUrl, codeVerifier, expectedState, mcpSessionId) {
    const config = await this.getConfig()
  ...
  async getUserInfo(accessToken) {
    const config = await this.getConfig()
    return client.fetchUserInfo(config, accessToken, client.skipSubjectCheck)
  }
   ...
   *
   * @returns {Promise<Object>} Token response with access_token, expires_in, scope
   */
   ...
   * @returns {Promise<{active: boolean, sub?: string, scope?: string}>} Introspection response
   */
   ...
   * @param {Object} result - Introspection result
   * @private
   */
  _cacheIntrospection(token, result) {
   ...
   */
  async getValidAccessToken(mcpSessionId) {
