 ...
 */
function inferColumns(ModelClass) {
...
/**
 * Generate a list view schema from a model class.
 *
 * @param {Object} ModelClass - Model class with static attributes, searchable, endpoint
 * @returns {{ model: string, title: string, columns: Object[], searchFields: string[], endpoint: string }}
 */
/**
 * Get all selectable column names for a model.
 *
 * @param {Object} ModelClass - Model class with static attributes
 * @returns {string[]} Column names available for selection
 */
export function getAvailableColumnNames(ModelClass) {
 ...
 *
 * @param {Object} schema - Schema from generateListSchema
 * @param {string[]|undefined} selectedColumns - Column names to keep (undefined = use defaults)
 * @param {Object} ModelClass - Model class (may have static defaultColumns)
 * @returns {Object} New schema with filtered columns
 */
export function applyColumnSelection(schema, selectedColumns, ModelClass) {
  ...
  const filtered = columnsToUse.map((name) => columnMap.get(name)).filter(Boolean)
...
export function generateListSchema(ModelClass) {
