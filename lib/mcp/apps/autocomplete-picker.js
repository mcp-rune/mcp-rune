 ...
 *
 * @param {Object} modelClasses - Map of model name → ModelClass
 * @returns {Object} Map of entity type → model registry key
 */
function buildTypeToModelMap(modelClasses) {
  const typeToModel = {}
 ...
 * @param {Object} [options.searchGroups={}] - Map of group name → group config (for cross-model search)
 * @param {string} options.namespace - Server namespace
 * @returns {Object[]} App definitions for AppRegistry
 */
export function createAutocompletePickerApp({ modelClasses, searchGroups = {}, namespace }) {
  ...
  if (hasGroups) {
    ...
    toolInputSchema.model = z.enum(modelNames).describe('Model to search')
  }
  ...
  const autocompleteTool = {
    ...
    async handleToolCall(args = {}, { apiClient, searchClient } = {}) {
      const { model, group, query, limit = 10 } = args
        ...
        if (searchClient && query) {
          try {
            ...
            results = records.map((record) => ({
              id: record.id,
              display: record.name || record.title || `ID: ${record.id}`,
      ...
      const searchCapability = SearchClient.getSearchCapability(ModelClass)
      let results = []

      if (searchClient && query) {
        try {
          const { records } = await searchClient.search(ModelClass, query, { perPage: limit })
          results = records.map((record) => {
