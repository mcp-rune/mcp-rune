 ...
 */
export function createSearchViewApp({ modelClasses, namespace }) {
  ...
  const searchTool = {
    ...
    toolInputSchema: {
      model: z.enum(modelNames).describe('Model to display'),
    ...
    async handleToolCall(args = {}, { searchClient } = {}) {
      const { model, query, filters = {}, page = 1, per_page = MAX_VIEW_PER_PAGE } = args
      ...
      let pagination = { page, per_page: clampedPerPage, total: 0 }
      const hasFilters = Object.keys(filters).length > 0

      if (searchClient) {
        try {
          if (query || hasFilters) {
            const result = await searchClient.search(ModelClass, query || null, {
              page,
            ...
            const result = await searchClient.list(ModelClass, {
              page,
              perPage: clampedPerPage
            })
            records = result.records
  ...
  const selectionTools = createSelectionTools('select_search_records', resourceUri, modelNames, {
    getHtml
  })
