 ...
 *
 * ```js
 * // Default — works for any model without customization
 * static displayAdapter = new DisplayAdapter()
 *
 * // Custom — override specific methods
 * class SchedulingDisplayAdapter extends DisplayAdapter {
 *   getEnumHints(fieldName, value) { ... }
 * }
 * static displayAdapter = new SchedulingDisplayAdapter()
 * ```
 */
...
export class DisplayAdapter {
   ...
   * @returns {string[]|null} Column names, or null for "show all"
   */
  getDefaultColumns(ModelClass) {
   ...
   *
   * @param {string} _fieldName - Attribute name (e.g., 'rights_status')
   * @param {string} _value - Enum value (e.g., 'cleared')
   * @returns {{ icon?: string, className?: string }|null}
   */
  getEnumHints(_fieldName, _value) {
    return null
  }
   ...
   *
   * @param {Object} ModelClass - Model class with static `attributes`
   * @returns {Object[]} Array of field definitions (name, label, type, ...)
   */
  inferDetailFields(ModelClass) {
    const attrs = ModelClass.attributes || {}
    const fields = []
   ...
   * @protected
   */
  _buildDetailField(name, attr, ModelClass) {
    const field = {
      name,
      label: attr.label || humanize(name),
      type: attr.type || 'string',
