...
export class GetPromptGuideTool extends BaseStrategyTool {
  ...
  getUsageRules() {
    ...
    const formRegistry = this.serverContext?.formRegistry
    const formModels = formRegistry
      ? Object.entries(this.models || {})
          .filter(([name]) => formRegistry.hasForm(name))
          .map(([name]) => name)
      : []
    const hasFormModels = this.serverContext?.appsEnabled && formModels.length > 0
...
After calling this tool, follow the mode-specific workflow in the documentation.`
  ...
  async execute(args) {
    const { guide_name, mode = 'guided', parent_type, parent_id } = args
    ...
    const getPromptInstance =
      this.promptRegistry.getPromptInstance ||
      ((name, pArgs) => {
        const PromptClass = this.promptRegistry.getPromptClass(name)
        return PromptClass ? new PromptClass(pArgs) : null
      })

    const getUnknownPromptError =
      this.promptRegistry.getUnknownPromptError || ((name) => `Unknown prompt: ${name}`)
