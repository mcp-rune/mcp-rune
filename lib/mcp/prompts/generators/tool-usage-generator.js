 ...
 * @returns {string} Markdown tool usage section
 */
export function generateToolUsage(context, instanceOverrides = {}) {
  const { promptClass, modelName } = context
  const config = { ...promptClass.toolUsage, ...instanceOverrides }
 ...
 */
function renderSingleMode(modelName, config, fieldDefs) {
  const hasPostCreate = config.postCreateSteps?.length > 0
  const parts = []
  ...
  // Exclusion notes
  if (config.excludeFromAttributes?.length > 0) {
 ...
 */
function renderVariantsMode(modelName, config, fieldDefs) {
  const parts = []
  ...
  // Post-create steps
  if (config.postCreateSteps?.length > 0) {
    config.postCreateSteps.forEach((step, i) => {
      parts.push(renderPostCreateStep(step, i + 2, modelName))
    })
  }
 ...
 */
function renderCreateBlock(modelName, config, fieldDefs) {
...
/**
 * Render alternative creation pattern (e.g., using _link in attributes).
 */
function renderAlternativeCreation(modelName, alt) {
  const title = alt.title || 'Alternative: Using link in attributes'
  const attrs = { ...alt.exampleAttributes }
 ...
 */
function deriveExampleAttributes(config, fieldDefs) {
  const attrs = {}
  ...
  for (const [name, def] of Object.entries(fieldDefs)) {
    ...
    } else if (def.required && def.examples?.length > 0) {
 ...
 */
function renderAttributeChecklists(config, fieldDefs) {
  const exclude = new Set(config.excludeFromAttributes || [])
  const parts = []
 ...
 */
function renderExclusionNotes(excludeList) {
 ...
 */
function renderImmutableNotes(fieldDefs) {
 ...
 */
function renderPostCreateStep(step, stepNum, modelName) {
  const parts = []
