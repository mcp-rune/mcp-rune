---
description: Invoke when the developer explicitly asks to turn the current conversation into a GitHub epic (e.g. via `/epic`, "open an epic for this", "create an epic issue and sub-issues for these findings"). Bundles everything gathered in the thread into ONE epic issue against `mcp-rune/mcp-rune`, then opens one focused sub-issue per concrete change, each linked back to the epic. Do NOT auto-fire on review/audit/refactor keywords — only when the user explicitly asks to file the epic.
argument-hint: [--all]
...
This command automates the **opening** step: take whatever the current conversation has converged on — a code review, a design exploration, an audit — and file the epic + its sub-issues against `mcp-rune/mcp-rune`.
...
The "file all sub-issues now" variant — passed as `--all` via `$ARGUMENTS`, or explicit "open the epic and all sub-issues" — is only correct when the user has already settled every axis and wants a complete checklist immediately.
...
Refresh the taxonomy first — it moves:

```sh
unset GITHUB_TOKEN
gh label list --repo mcp-rune/mcp-rune --limit 200
```

Default scoped labels per area:
...
Only when the user explicitly opts in (`--all` in `$ARGUMENTS`, or "open the epic and all sub-issues"), additionally:
...
- Don't name downstream implementors (mcp-servers-mgx, engineer-mcp) in issue bodies. Describe their _shape_ instead.
