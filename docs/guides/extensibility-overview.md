...
> **Projection-layer rule.** Tools, apps, prompts, and domain workflows consume only the `DataLayer` interface — never `SearchService`, `ApiClient`, or `ModelService` directly. See [The Projection-Layer Rule](./data-layer-guide.md#the-projection-layer-rule).
...
| Surface                                | What it owns                                                                                                                                                                        | Guide                                                                                                                                                                         |
| -------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`ApiExtension`**                     | New MCP tools tied to the model layer (custom verbs like publish/archive, bulk ops, RPC). Optionally contribute `ModelService` mixins and `SummaryStrategy`s for `analysis_ingest`. | [API Extensions](./api-extensions.md) · [Summary Strategies](./summary-strategies.md) · [Authoring Extensions](./authoring-extensions-guide.md)                               |
