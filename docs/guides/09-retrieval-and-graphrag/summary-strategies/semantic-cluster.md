...
Complementary to the SQL `cluster` stratifier (see [analysis-quickstart](../analysis-quickstart.md) section on sampling): that one partitions a sample server-side; this one summarizes a page client-side and writes a narrative cluster summary to `analysis_memories`.
...
- [SQL `cluster` stratifier](../analysis-quickstart.md#sample-mode-with-graph-stratifiers) on `analysis_query mode:"sample"` — server-side clustering for sampling. Same underlying idea, different output (a sample vs. a narrative summary).
