...
The previous chapter covered single-call tools. This one covers operations that span multiple LLM turns — fetch context, analyze, decide, write. mcp-rune offers `WorkflowDefinition` files that the LLM executes one step at a time via the `suggest_workflow` and `get_workflow_step` tools.

A workflow is itself a declaration (the recurring shape in mcp-rune): you write the _what_ and the _why_ for each step, the framework runs the orchestration. The LLM sees a deterministic sequence with the right `contextHints` at each step, instead of having to invent the orchestration itself.

This is the last chapter of Part I that covers Tools. Apps — the human-facing iframe UIs that consume the same three layers — pick up in [chapter 5](../05-apps/).
...
| ---------------- | ---------------- | ----------------------------- |
| `list_models`    | `find_model_app` | "list", "show", "browse"      |
| `search_records` | `find_model_app` | "search", "find", "filter"    |
...
Data tools (`list_models`, `search_records`, `find_records`) return raw JSON for programmatic use. View tools (`find_model_app`, `show_model_app`, `view_selection_app`) render interactive MCP Apps. Workflows need data, not UI.
...
// WRONG — "browse" semantically matches find_model_app
