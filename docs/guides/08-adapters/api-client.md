...
If you're using [`createDefaultAppRegistry`](../04-apps-search-forms/mcp-apps.md), pass `createApiClient` as one of its options — it threads through to both internal registries.
...
When OAuth is enabled, the token comes from the [`OAuthService`](../06-auth-and-transport/oauth2-discovery.md) — you don't fetch it yourself in the factory. `ToolRegistry.registerTools` accepts a `getAccessToken` callback that closes over the request context:
...
Tools and apps receive an `ApiClient`. They should not import a concrete client class, sniff for axios, or call `baseUrl` to construct URLs themselves — the [`EndpointResolver`](../03-tools-and-services/service-layer.md) owns URL composition. The whole point of the `ApiClient` seam is that every consumer is interchangeable.
...
3. Add it to an [ApiExtension](../09-extensions/api-extensions.md) (new tools, `ModelService` mixins).
...
- [Service Layer Guide](../03-tools-and-services/service-layer.md) — how `ModelService` composes `ApiClient` + `EndpointResolver` + convention.
- [OAuth 2.0 Discovery Flow](../06-auth-and-transport/oauth2-discovery.md) — where the token in `createApiClient(token)` comes from.
