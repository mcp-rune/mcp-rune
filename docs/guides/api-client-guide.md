...
If you're using [`createDefaultAppRegistry`](./mcp-apps-guide.md), pass `createApiClient` as one of its options — it threads through to both internal registries.
...
For stub testing at the higher `DataLayer` boundary (so you don't have to mock the wire format), see the [In-Memory DataLayer](./data-layer-guide.md#in-memory-stub-for-tests) section instead — it bypasses both `ApiClient` and the convention.
...
Tools and apps receive an `ApiClient`. They should not import a concrete client class, sniff for axios, or call `baseUrl` to construct URLs themselves — the [`EndpointResolver`](./service-layer-guide.md) owns URL composition. The whole point of the `ApiClient` seam is that every consumer is interchangeable.
...
3. Add it to an [ApiExtension](./api-extensions.md) (new tools, `ModelService` mixins).
...
- [DataLayer Guide](./data-layer-guide.md) — the layer above `ApiClient` that adds model awareness.
- [Custom API Convention](./api-convention-guide.md) — response normalization and payload wrapping live here, not in the client.
- [Service Layer Guide](./service-layer-guide.md) — how `ModelService` composes `ApiClient` + `EndpointResolver` + convention.
- [OAuth 2.0 Discovery Flow](./oauth2-discovery-flow.md) — where the token in `createApiClient(token)` comes from.
