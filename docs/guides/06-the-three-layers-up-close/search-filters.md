...
The previous chapter covered the request shape — how `{ query, filters, page, perPage }` gets translated into your backend's wire format. This chapter covers the _typed filter contract_ the LLM uses to build that `filters` object: `text`, `enum`, `relation`, `date_range`, `integer_range`. Both sides of an integration matter — the model-side filter declarations the framework consumes, and the backend-side endpoint that has to honour them. The running example here uses Rails for concreteness; the contract itself is backend-agnostic.
...
> **Prerequisite:** the `search_records` and `get_filters_guide` MCP tools are contributed by the opt-in `searchExtension` (v0.45.0+). Without registering the extension on `ToolRegistry`, those tools are absent from your server even if every model declares filters. See [Authoring Extensions Guide](../10-extensions/authoring-extensions.md) for the full pattern; the short version is:
>
> ```ts
...
> **Note:** This guide covers **direct search** — models with their own search endpoint. For **group search** (multiple models sharing one endpoint, e.g., Library search), see the Search Adapters section in the [Model service](./model-service.md). Group search also supports filter pass-through via the same adapter pipeline.
