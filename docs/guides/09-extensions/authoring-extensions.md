...
> **Projection-layer rule.** Apps, tools, prompts, and domain workflows consume only the `DataLayer` interface — never `SearchService`, `ApiClient`, or `ModelService` directly. See [The Projection-Layer Rule](../08-adapters/data-layer.md#the-projection-layer-rule).
...
- [Service Layer Guide](../03-tools-and-services/service-layer.md) — what `ModelService.dispatch`, `buildPayload`, `endpointResolver`, `apiClient`, `models` actually do — the stable mixin contract
