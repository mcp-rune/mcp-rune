...
For the protocol-level deep dive — how iframe communication works, message ordering, the `@modelcontextprotocol/ext-apps` SDK — see [MCP Apps Architecture](../04-apps-search-forms/mcp-apps-arch.md). For the user-facing introduction, see [MCP Apps Guide](../04-apps-search-forms/mcp-apps.md).
...
- [MCP Apps Guide](../04-apps-search-forms/mcp-apps.md) — user-facing overview of the apps system.
- [MCP Apps Architecture](../04-apps-search-forms/mcp-apps-arch.md) — protocol-level reference for iframe ↔ host communication.
- [Attribute Kinds](../02-prompt-dsl/attribute-kinds.md) — the kind taxonomy custom apps inherit.
- [Model Form Customization](../04-apps-search-forms/model-form.md) — layout primitives for the generic `model-form` (often enough; you may not need a custom app).
