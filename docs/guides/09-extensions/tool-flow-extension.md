...
- [Authoring Extensions](./authoring-extensions.md) — covers `HttpExtension` and `ApiExtension`. `ToolFlowExtension` is the third sibling.
- [Extensions](./extensions-http.md) — `HttpExtension` reference.
- [API Extensions](./api-extensions.md) — `ApiExtension` reference.
- [MCP Apps Guide](../04-apps-search-forms/mcp-apps.md) — `AppDefinition` shape (what `registerTool` accepts).
