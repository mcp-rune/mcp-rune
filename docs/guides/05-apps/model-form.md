...
The default app registry synthesizes a `BaseAppForm` for every model — fine for most cases. This chapter is for when the synthesized form doesn't match the UX you want: horizontal grids, side-by-side field groups, per-fieldGroup overrides, stacked variants. You extend `BaseAppForm` (from `src/mcp/apps/lib/base-app-form.ts`) and supply it via the registry's `formClasses:` map.
...
- **[Prompt Creation Guide](../03-the-prompt/prompt-creation.md)** — Configuring `fieldGroups`, `sections`, and prompt content
