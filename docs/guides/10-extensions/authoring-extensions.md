...
> **Projection-layer rule.** Apps, tools, prompts, and domain workflows consume only the `DataLayer` interface — never `SearchService`, `ApiClient`, or `ModelService` directly. See [The Projection-Layer Rule](../06-the-three-layers-up-close/data-layer.md#the-projection-layer-rule).
