...
→ Full references: [Tool Creation](../04-tools/tool-creation.md) for the `BaseTool` contract, [API Extensions](./api-extensions.md) for the registration shape.
...
→ Full reference: [Custom API Convention](../06-the-three-layers-up-close/api-convention.md).
...
→ Full reference: [DataLayer Guide](../06-the-three-layers-up-close/data-layer.md), §"In-Memory Stub for Tests."
