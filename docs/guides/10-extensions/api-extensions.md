...
This guide is the **conceptual reference**: what an `ApiExtension` is, what the framework guarantees, what the registration and reading contracts look like, and why the design choices are what they are. For a **step-by-step walkthrough of authoring your own extension**, see [Authoring Extensions Guide](./authoring-extensions.md).
...
It is the model-layer parallel of [`HttpExtension`](./extensions-http.md) and follows the same authoring contract.
...
API extensions contribute capabilities at the `DataLayer` seam: MCP tools, mixin methods on the data layer that compose `EndpointResolver` and `ApiClient`, configuration that lives on individual models. They are **not** the place for HTTP-layer concerns (routes, route-scoped middleware) — those belong to [`HttpExtension`](./extensions-http.md).
...
See [Summary Strategies](../09-retrieval-and-graphrag/summary-strategies.md) for the full catalog of built-ins, the strategy interface, and an authoring walkthrough.
