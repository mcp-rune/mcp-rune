 ...
 *
 * This follows the same pattern as selection-tools.ts:
...
export function createFormDataTools(
  ...
  const collectTool: AppToolDefinition = {
    resourceUri,
  ...
  const getFormDataTool: AppToolDefinition = {
    ...
    async handleToolCall(
      ...
      if (args.model) {
        const entry = formDataStore.get(args.model as string)
        if (!entry) {
          return {
            content: [
              {
